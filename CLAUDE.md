...
- **MCP**: Client resolves server names → portable definitions; model translates to native format
...
### MCP Server Catalog
The client layer owns MCP server resolution ("client resolves, model translates"):
```java
// Build catalog from JSON file or programmatically
...
**All development plans MUST use the Forge Methodology roadmap format** from `~/research/forge-methodology/templates/ROADMAP-TEMPLATE.md`. Plans are stored in the `plans/` directory.

**MANDATORY: All plan documents, design documents, and roadmaps MUST be persisted under `plans/`** — never in `~/.claude/` or any other location. The `plans/` directory is the single source of truth for all significant development work. This includes:
- Roadmap files (`plans/*-ROADMAP.md`)
- Design documents (`plans/*-DESIGN.md`)
- Learnings (`plans/learnings/`)
- Knowledge base (`plans/knowledge/`)
- Any reference material for ongoing work

Key requirements:
- Steps have **entry criteria**, **work items**, and **exit criteria**
- Every step exit includes: tests pass, learnings doc created, CLAUDE.md updated, ROADMAP.md checkboxes updated, commit
- Stages group related steps; steps are numbered `Stage.Step` (e.g., 1.1, 1.2)
- Work items use action verbs: IMPLEMENT, CREATE, WRITE, VERIFY, UPDATE, DELETE
- Commit format: `Step X.Y: Brief description`
- Not using forge-roadmap format is the **exception**, not the rule — only skip for trivial single-commit fixes
...
Accountable for agent-client project health, provider SDK currency, roadmap execution, and knowledge base maintenance.

## KB Navigation

Steward knowledge lives in `plans/knowledge/`:

| If you need... | Start here |
|----------------|-----------|
| Architecture decisions, design philosophy | `plans/knowledge/architecture/index.md` |
| Development state, roadmap, known issues | `plans/knowledge/development/index.md` |
| Project monitoring, issues, PRs | `plans/knowledge/monitoring/index.md` |
| CLI agent landscape (external) | agentic-patterns-research: `papers/summaries/agentic-cli-tools-2026-landscape.md` |
| DevNexus talk context (external) | tuvium-devnexus-2026: `CLAUDE.md` |
...
- Follow the formatting mandate: `./mvnw spring-javaformat:apply` before every commit
- Update KB (`plans/knowledge/`) as work progresses

### Stewardship Mode
**Triggered when**: user asks to check status, review health, or plan work.
- Run health checklist (below)
...
### Owns
- Build health, test suite, quality infrastructure for agent-client
- Knowledge base at `plans/knowledge/`
- Roadmap execution (Steps 1, 2a, 2b-1, 2b-2, 3 from `plans/ROADMAP.md`)
- Provider SDK integration (Claude, Gemini, SWE-agent)
- Tracking upstream CLI releases and API changes
